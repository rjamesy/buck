# Buck — Codex ↔ ChatGPT Review Bridge
...
# Send inline text via stdin (preferred — avoids shell quoting issues)
...
- **If GPT gives feedback**, revise the plan and resend to Buck. Do not involve the user unless GPT and Codex cannot agree after 3 rounds on the same point.

## Reliability notes

- The script handles retries internally (default 2 retries on error/tool-use). No need to retry from Codex.
- Use `timeout: 900000` (15 min) on the Bash tool call — covers Buck's response timeout + retry headroom.
...
```bash
# Speak only
"$HOME/Mac Projects/buck/buck-speak.sh" --speak --text "Hello from Codex"
