...
final class CodexBridge: BridgeProtocol {
    ...
    private func loadConfig() {
        let configPath = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".buck/codex-config.json")
    ...
    func waitForResponse(timeout: TimeInterval) async throws -> String {
        ...
        if text.isEmpty {
            throw CodexBridgeError.apiError("Empty response from Codex API")
        }
    ...
    private func classifyError(statusCode: Int, error: CodexAPIErrorDetail) -> String {
        ...
        if statusCode == 401 || type.contains("auth") || type.contains("invalid_api_key") {
            return "Auth error: \(message). Check api_key in ~/.buck/codex-config.json"
...
enum CodexBridgeError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noAPIKey:
            return "No OpenAI API key. Set api_key in ~/.buck/codex-config.json or OPENAI_API_KEY env var."
        case .noMessage:
